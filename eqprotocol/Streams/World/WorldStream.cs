...
namespace EQProtocol.Streams.World
{
    public class WorldStream : EQStream
    {
        ...
        public event EventHandler<EnterWorld> EnterWorld;
        ...
        protected override string GetOpcodeNameForLogging(ushort opcode)
        {
            ...
            {
                return Enum.GetName(typeof(WorldOp), opcode);
            }
        ...
        {
            Logger.Debug($"World packet: {(WorldOp)packet.Opcode}");
        ...
        public void SendNameApproval(ApproveName nameApproval)
        {
            Send(AppPacket.Create(WorldOp.ApproveName, nameApproval));
        }

        public void SendCharacterCreate(CharacterCreate charCreate)
        {
