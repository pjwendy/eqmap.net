...
namespace EQProtocol.Streams.Common.UnUsed_Packets {
	...
	/// </summary>
	public struct SharedTaskRemovePlayer : IEQStruct {
				// No properties - structure definition not found or empty

		/// <summary>
		/// Initializes a new instance of the SharedTaskRemovePlayer struct with specified field values.
		/// </summary>
		// C# 8.0: Parameterless struct constructors not supported - uncomment if using C# 10.0+
		// public SharedTaskRemovePlayer() : this() {
		...
		public SharedTaskRemovePlayer(byte[] data, int offset = 0) : this() {
			Unpack(data, offset);
		}
		...
		public SharedTaskRemovePlayer(BinaryReader br) : this() {
			Unpack(br);
		}
		...
		public override string ToString() {
			var ret = "struct SharedTaskRemovePlayer {\n";
						ret += "	// No properties\n";
			ret += "}";
