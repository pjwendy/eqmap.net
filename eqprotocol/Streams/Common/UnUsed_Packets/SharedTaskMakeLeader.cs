...
namespace EQProtocol.Streams.Common.UnUsed_Packets {
	...
	/// </summary>
	public struct SharedTaskMakeLeader : IEQStruct {
				// No properties - structure definition not found or empty

		/// <summary>
		/// Initializes a new instance of the SharedTaskMakeLeader struct with specified field values.
		/// </summary>
		// C# 8.0: Parameterless struct constructors not supported - uncomment if using C# 10.0+
		// public SharedTaskMakeLeader() : this() {
		...
		public SharedTaskMakeLeader(byte[] data, int offset = 0) : this() {
			Unpack(data, offset);
		}
		...
		public SharedTaskMakeLeader(BinaryReader br) : this() {
			Unpack(br);
		}
		...
		public override string ToString() {
			var ret = "struct SharedTaskMakeLeader {\n";
						ret += "	// No properties\n";
			ret += "}";
