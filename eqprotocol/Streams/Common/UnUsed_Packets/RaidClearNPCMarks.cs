...
namespace EQProtocol.Streams.Common.UnUsed_Packets {
	...
	/// </summary>
	public struct RaidClearNPCMarks : IEQStruct {
		/// <summary>
		...
		public RaidClearNPCMarks(uint TargetID, uint Number, byte[] Name) : this() {
			Targetid = TargetID;
			Number = Number;
			Name = Name;
		}
		...
		public RaidClearNPCMarks(byte[] data, int offset = 0) : this() {
			Unpack(data, offset);
		}
		...
		public RaidClearNPCMarks(BinaryReader br) : this() {
			Unpack(br);
		}
		...
		public override string ToString() {
			var ret = "struct RaidClearNPCMarks {\n";
			ret += "	Targetid = ";
			try {
