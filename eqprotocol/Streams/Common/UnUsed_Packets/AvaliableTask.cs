...
namespace EQProtocol.Streams.Common.UnUsed_Packets {
	...
	/// </summary>
	public struct AvaliableTask : IEQStruct {
				// No properties - structure definition not found or empty

		/// <summary>
		/// Initializes a new instance of the AvaliableTask struct with specified field values.
		/// </summary>
		// C# 8.0: Parameterless struct constructors not supported - uncomment if using C# 10.0+
		// public AvaliableTask() : this() {
		...
		public AvaliableTask(byte[] data, int offset = 0) : this() {
			Unpack(data, offset);
		}
		...
		public AvaliableTask(BinaryReader br) : this() {
			Unpack(br);
		}
