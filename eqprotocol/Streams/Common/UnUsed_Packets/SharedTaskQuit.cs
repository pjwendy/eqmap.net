...
namespace EQProtocol.Streams.Common.UnUsed_Packets {
	...
	/// </summary>
	public struct SharedTaskQuit : IEQStruct {
				// No properties - structure definition not found or empty

		/// <summary>
		/// Initializes a new instance of the SharedTaskQuit struct with specified field values.
		/// </summary>
		// C# 8.0: Parameterless struct constructors not supported - uncomment if using C# 10.0+
		// public SharedTaskQuit() : this() {
		...
		public SharedTaskQuit(byte[] data, int offset = 0) : this() {
			Unpack(data, offset);
		}
		...
		public SharedTaskQuit(BinaryReader br) : this() {
			Unpack(br);
		}
