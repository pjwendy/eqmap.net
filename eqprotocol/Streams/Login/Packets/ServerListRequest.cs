...
namespace EQProtocol.Streams.Login.Packets {
	...
	/// </summary>
	public struct ServerListRequest : IEQStruct {
				// No properties - structure definition not found or empty

		/// <summary>
		/// Initializes a new instance of the ServerListRequest struct with specified field values.
		/// </summary>
		// C# 8.0: Parameterless struct constructors not supported - uncomment if using C# 10.0+
		// public ServerListRequest() : this() {
		...
		public ServerListRequest(byte[] data, int offset = 0) : this() {
			Unpack(data, offset);
		}
		...
		public ServerListRequest(BinaryReader br) : this() {
			Unpack(br);
		}
		...
		public override string ToString() {
			var ret = "struct ServerListRequest {\n";
						ret += "	// No properties\n";
			ret += "}";
