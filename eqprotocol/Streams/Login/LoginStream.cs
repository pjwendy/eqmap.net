...
using EQProtocol.Streams.Login.Packets;
using EQProtocol.Streams.Common;
using OpenEQ.Netcode;
using static System.Text.Encoding;
using static OpenEQ.Netcode.Utility;

namespace EQProtocol.Streams.Login
{
    public class LoginStream : EQStream
    {
        ...
        protected override string GetOpcodeNameForLogging(ushort opcode)
        {
            ...
            {
                return Enum.GetName(typeof(LoginOp), opcode);
            }
        ...
        protected override void HandleAppPacket(AppPacket packet)
        {
            switch ((LoginOp)packet.Opcode)
            {
                case LoginOp.ChatMessage:
                    Send(AppPacket.Create(LoginOp.Login, new Packets.Login(), CryptoBlob));
                    break;
                ...
                case LoginOp.ServerListResponse:
                    var header = packet.Get<ServerListResponse>();
                    ServerList?.Invoke(this, header.Servers);
                    break;
                case LoginOp.PlayEverquestResponse:
                    var resp = packet.Get<PlayEverquestResponse>();
                    ...
                    break;
                default:
                    Logger.Debug($"Unhandled packet in LoginStream: {(LoginOp)packet.Opcode} (0x{packet.Opcode:X04})");
        ...
        {
            CurPlay = server;
            Send(AppPacket.Create(LoginOp.PlayEverquestRequest, new PlayEverquestRequest(5, server.RuntimeID)));
        }
    }
}