...
namespace OpenEQ.Netcode {
	...
	/// </summary>
	public struct ReqNewZone : IEQStruct {
		/// <summary>
		...
		public ReqNewZone(byte[] char_name, byte[] zone_short_name, byte[] zone_long_name, byte ztype, byte[] fog_red, byte[] fog_green, byte[] fog_blue, byte unknown323, float[] fog_minclip, float[] fog_maxclip, float gravity, byte time_type, byte[] rain_chance, byte[] rain_duration, byte[] snow_chance, byte[] snow_duration, byte[] unknown537, byte sky, byte[] unknown571, float zone_exp_multiplier, float safe_y, float safe_x, float safe_z, float min_z, float max_z, float underworld, float minclip, float maxclip, byte[] unknown620, byte[] zone_short_name2, int unknown800, byte[] unknown804, int unknown844, int unknown848, ushort zone_id, ushort zone_instance, uint scriptNPCReceivedanItem, uint bCheck, uint scriptIDSomething, uint underworld_teleport_index, uint scriptIDSomething3, uint suspend_buffs, uint lava_damage, uint min_lava_damage, byte unknown888, byte unknown889, byte unknown890, byte unknown891, byte unknown892, byte unknown893, byte fall_damage, byte unknown895, uint fast_regen_hp, uint fast_regen_mana, uint fast_regen_endurance, uint unknown908, uint unknown912, float fog_density, uint unknown920, uint unknown924, uint unknown928, byte[] unknown932) : this() {
			CharName = char_name;
		...
		public ReqNewZone(byte[] data, int offset = 0) : this() {
			Unpack(data, offset);
		}
		...
		public ReqNewZone(BinaryReader br) : this() {
			Unpack(br);
		}
