...
namespace OpenEQ.Netcode {
	...
	/// </summary>
	public struct WhoAllRequest : IEQStruct {
		/// <summary>
		...
		public WhoAllRequest(byte[] whom, uint wrace, uint wclass, uint lvllow, uint lvlhigh, uint gmlookup, uint guildid, byte[] unknown088, uint type) : this() {
			Whom = whom;
			Wrace = wrace;
			Wclass = wclass;
			Lvllow = lvllow;
		...
		public WhoAllRequest(byte[] data, int offset = 0) : this() {
			Unpack(data, offset);
		}
		...
		public WhoAllRequest(BinaryReader br) : this() {
			Unpack(br);
		}
