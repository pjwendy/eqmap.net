...
namespace OpenEQ.Netcode {
	...
	/// </summary>
	public struct RecipeAutoCombine : IEQStruct {
		/// <summary>
		...
		public RecipeAutoCombine(uint window, uint invslot, uint type, uint target_id, byte can_cast, byte can_scribe, byte[] txtfile) : this() {
			Window = window;
			Invslot = invslot;
			Type = type;
			TargetId = target_id;
		...
		public RecipeAutoCombine(byte[] data, int offset = 0) : this() {
			Unpack(data, offset);
		}
		...
		public RecipeAutoCombine(BinaryReader br) : this() {
			Unpack(br);
		}
		...
		public override string ToString() {
			var ret = "struct RecipeAutoCombine {\n";
			ret += "	Window = ";
			try {
